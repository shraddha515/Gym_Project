...
.filter-bar {
    background: var(--card-bg); /* Use a subtle background */
...
.input-group .form-control {
    border-color: #ced4da;
    background-color: #f8f9fa; /* Lighter background for inputs */
    color: var(--text-dark);
}
...
@media (min-width: 768px) {
    .search-group {
        max-width: 250px; /* Reduces width of search input */
    }
    .date-group {
        max-width: 180px; /* Reduces width of date inputs */
    }
}
...
.btn-group-responsive {
    display: flex;
    flex-wrap: wrap;
    gap: 8px; /* Spacing between buttons on mobile */
...
.modal-header .btn-close {
    filter: invert(1) grayscale(1) brightness(2); /* Makes the close button white */
}
...
<div class="container py-4">
    ...
    <div class="input-group search-group flex-grow-1 me-md-2 mb-2 mb-md-0">
        ...
        <input type="text" id="searchKeyword" placeholder="Search category/description" value="{{ $q ?? '' }}" class="form-control border-start-0" autocomplete="off">
    </div>
    ...
    <div class="btn-group-responsive">
        ...
        <a href="{{ route('expenses.export.csv', ['from' => $from, 'to' => $to]) }}" class="btn btn-outline-secondary me-2">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title" id="addExpenseModalLabel"><i class="bi bi-plus-circle-fill me-2"></i>Add New Expense</h5>
            ...
            <div class="modal-body">
                <form method="POST" action="{{ route('expenses.store') }}">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" id="category" name="category" class="form-control form-control-sm" value="{{ old('category') }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="amount" class="form-label">Amount</label>
                        <input type="number" step="0.01" id="amount" name="amount" class="form-control form-control-sm" value="{{ old('amount') }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="expense_date" class="form-label">Expense Date</label>
                        <input type="date" id="expense_date" name="expense_date" class="form-control form-control-sm" value="{{ old('expense_date', date('d-m-y')) }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="payment_method" class="form-label">Payment Method</label>
                        <input type="text" id="payment_method" name="payment_method" class="form-control form-control-sm" value="{{ old('payment_method') }}">
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="d-grid gap-2">
                        <button type="submit" class="btn btn-success mt-2"><i class="bi bi-floppy-fill me-2"></i>Save Expense</button>
                    </div>
                </form>
            </div>
        </div>
                    ...
                    <tbody>
                        @forelse($expenses as $e)
                            <tr>
                                ...
                                <td class="text-truncate" style="max-width: 150px;">{{ $e->description }}</td>
                                <td class="text-nowrap">
                                    <a href="{{ route('expenses.edit', $e->id) }}" class="btn btn-sm btn-outline-primary" title="Edit">
                                        <i class="bi bi-pencil-square"></i>
                                    </a>
                                    <form method="POST" action="{{ route('expenses.destroy', $e->id) }}" class="d-inline" onsubmit="return confirm('Delete this expense?');">
                                        @csrf
                                        @method('DELETE')
...
<div class="col-12 d-block d-md-none">
    <h5 class="mb-3">Expenses List <span class="badge bg-primary rounded-pill small ms-2">Total: {{ number_format($total, 2) }}</span></h5>
    @forelse($expenses as $e)
        <div class="card mb-3 shadow-sm expenses-card-item">
            <div class="card-body">
                ...
                <div class="d-flex justify-content-end gap-2 mt-3">
                    <a href="{{ route('expenses.edit', $e->id) }}" class="btn btn-sm btn-outline-primary"><i class="bi bi-pencil-square me-1"></i>Edit</a>
                    <form method="POST" action="{{ route('expenses.destroy', $e->id) }}" onsubmit="return confirm('Delete this expense?');">
                        @csrf
                        @method('DELETE')
