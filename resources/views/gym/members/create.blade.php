            ...
            <div class="row g-3">
                ...
                <div class="row g-3 mt-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" min="0" step="0.01" name="Discount" class="form-control"
                            value="{{ old('Discount', $member->Discount ?? '') }}"
                ...
                <div class="col-md-6">
                    ...
                    <select name="status" class="form-select form-select-sm">
                        <option value="Active" {{ isset($member) && $member->status == 'Active' ? 'selected' : '' }}>
                            Active</option>
                        <option value="Inactive"
                            {{ isset($member) && $member->status == 'Inactive' ? 'selected' : '' }}>
                            Inactive</option>
                    </select>
                </div>
...
document.addEventListener("DOMContentLoaded", function () {
    const membershipDropdown = document.querySelector("select[name='membership_type']");
    const feesDueInput = document.querySelector("input[name='fees_due']");
    const discountInput = document.querySelector("input[name='Discount']");
    const feesPaidInput = document.querySelector("input[name='fees_paid']");

    let originalFee = 0; // Membership ke hisaab se base fee
    let discountedFee = 0; // Discount ke baad wali fee

    // ✅ Membership change hone par base fee set karo
    membershipDropdown.addEventListener("change", function () {
        let selectedOption = this.options[this.selectedIndex];
        let fee = selectedOption.getAttribute("data-fee");

        if (fee) {
            originalFee = parseFloat(fee);
    ...
    // ✅ Discount apply hone par calculation
    discountInput.addEventListener("input", function () {
        applyDiscount();
    });

    // ✅ Fees Paid apply hone par calculation
    feesPaidInput.addEventListener("input", function () {
        applyDiscount();
    });

    function applyDiscount() {
        let discount = parseFloat(discountInput.value) || 0;
        let paid = parseFloat(feesPaidInput.value) || 0;

        // Step 1: discount ke baad wali fees nikaalo
        discountedFee = originalFee - discount;
        if (discountedFee < 0) discountedFee = 0;

        // Step 2: usme se paid fees minus karo
        let finalDue = discountedFee - paid;
        if (finalDue < 0) finalDue = 0;

        // ✅ Fees Due update karo
        feesDueInput.value = finalDue;
    }

    // Agar edit form mein membership already selected hai to trigger kar do
    if (membershipDropdown.value) {
        membershipDropdown.dispatchEvent(new Event("change"));
    }
});
</script>
